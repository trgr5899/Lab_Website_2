...
function loadStatsPage()
{
	var table = document.getElementById("stats_table");//Retrieve our table element
		var row_counter;//Keeps track of our row index//Keeps track of our column index
		var opposingteam;
		var cuscore;
		var wins=0;
		var losses=0;
		...
		{//Outer for loop iterates over each row
				...
				opposingteam= table.rows[row_counter].cells[1].innerHTML;//Read in a cells current value
				cuscore=parseInt(table.rows[row_counter].cells[2].innerHTML);
				opscore=parseInt(table.rows[row_counter].cells[3].innerHTML);
				if(cuscore>opscore)
				{
					...
					{
						table.rows[row_counter].cells[4].innerHTML = opposingteam;
						losses=losses+1;
					}
				}
				...
				//Update the actual html of the cell
